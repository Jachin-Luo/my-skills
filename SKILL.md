...
| 🎨 设计 | wechat-cover-workflow | — | 微信公众号封面、21:9封面、1:1封面、Swiss风格封面 |
| 📱 社交 | wewrite | — | 公众号、微信文章 |
| 📱 社交 | xiaohongshu-mcp | — | 小红书、发布笔记、小红书搜索 |
| 📱 社交 | xhs-writer-skill | — | 写小红书、小红书笔记、小红书图文、小红书种草 |
| 📱 社交 | guizang-social-card-skill | — | 小红书图文、社交卡片、微信公众号封面、瑞士风格、杂志风格 |
| 📱 社交 | comic-creator | — | 做个漫画、帮我画个漫画发小红书、创作漫画、四格漫画 |
| 📱 社交 | follow-builders | — | AI日报、每日讯息 |
| 🎨 创意 | image-style-prompt-extractor | — | 分析风格、反推提示词、提取美学、风格模板、复刻风格 |
| 🎨 创意 | image-prompt-stylist | — | 图像提示词、配图风格、图片prompt、画风匹配 |
| 📚 内容 | teach | — | 教我学、teach、学习XX、入门教程 |
| 📚 内容 | qiaomu-anything-to-notebooklm | — | 生成播客、做成PPT、思维导图、NotebookLM、深度分析 |
| 🔮 命理 | bazi | — | 八字、命理、四柱 |
...
**统计：** 7个分类，24个skill（1个父skill + 4个子skill + 19个独立skill）
...
从仓库拉取后，需要初始化 `catalog/skills.json`：

```bash
cd ~/.hermes/skills/my-skills
python3 scripts/list_skills.py init --root ~/.hermes/skills
```

这会扫描 `~/.hermes/skills` 下的所有 SKILL.md 并生成目录。之后可用 `show` 查看：
```bash
python3 scripts/list_skills.py show
```

**注意**：仓库中的 SKILL.md 是通用模板，需要根据本地环境定制触发词表格和分类。
